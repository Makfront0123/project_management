import { useAuthStore } from "@/features/auth/store/auth_store";
import axios from "axios";


const baseUrl = `${import.meta.env.VITE_API_BASE_URL}/api/v1`;

export const getPrivateMessages = async (teamId: string, fromId: string, toId: string) => {
    const response = await axios.get(`${baseUrl}/private-messages/${teamId}/${fromId}/${toId}`);
    return response.data;
};

export const getGlobalMessages = async (teamId: string) => {
    const response = await axios.get(`${baseUrl}/global-messages/${teamId}`);
    return response.data;
};

export const deleteGlobalMessages = async (teamId: string) => {
    const token = useAuthStore.getState().token;
    const response = await axios.delete(`${baseUrl}/global-messages/${teamId}`, {
        headers: {
            Authorization: `Bearer ${token}`,
        }
    });
    return response.data;
};

export const deletePrivateMessages = async (teamId: string, fromId: string, toId: string) => {
    const token = useAuthStore.getState().token;
    const response = await axios.delete(`${baseUrl}/private-messages/${teamId}/${fromId}/${toId}`, {
        headers: {
            Authorization: `Bearer ${token}`,
        }
    });
    return response.data;
};
